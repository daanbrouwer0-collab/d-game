class RobotRallyEngine {
  constructor() {
    ...
    /** @type {Record<string, string[]>} robotId → offered upgrade ids at match start */
  ...
  isP2pMode() {
    return this.gameMode === CONFIG.GAME_MODES.P2P;
  }
  ...
  exportGameState() {
    return {
      ...
      matchUpgradeOffers: Object.fromEntries(
        Object.entries(this.matchUpgradeOffers || {}).map(([robotId, ids]) => [
          robotId,
      ...
      robots: this.robots.map(robot => ({
        ...
        colors: { ...robot.colors },
        style: robot.style,
        peerUserId: robot.peerUserId || null,
        x: robot.x,
        y: robot.y,
        dir: robot.dir,
  ...
  importGameState(state) {
    ...
    this.matchUpgradeOffers = {};
    if (state.matchUpgradeOffers && typeof state.matchUpgradeOffers === 'object') {
      Object.entries(state.matchUpgradeOffers).forEach(([robotId, ids]) => {
        if (!robotId || !Array.isArray(ids)) return;
        this.matchUpgradeOffers[robotId] = ids.map((id) => String(id)).filter(Boolean);
      });
    }
    ...
    this.robots = state.robots.map(raw => {
      ...
      robot.peerUserId = raw.peerUserId || null;
  ...
  loadCourse(courseId, players, gameMode = CONFIG.GAME_MODES.HOTSEAT, playerCount = 2, boardData = null, options = {}) {
    ...
    const isMatrix = this.gameMode === CONFIG.GAME_MODES.P2P;
    const humanCount = isVsAi
      ? 1
      : Math.max(
        isMatrix ? (CONFIG.P2P?.MIN_PLAYERS || 2) : CONFIG.MIN_HOTSEAT_PLAYERS,
    ...
    for (let i = 0; i < humanCount; i++) {
      ...
      robot.peerUserId = profile.peerUserId || profile.userId || null;
  ...
  beginMatchReady() {
    ...
    this.robots.forEach((robot) => {
      ...
      if (robot.isBot) {
        if (options.length) {
          const pick = this.pickUpgradeForBot(robot, options);
          if (pick) this.applyUpgradeChoice(robot.id, pick.id);
        }
        return;
      }
      if (!options.length) {
        autoReadyIds.push(robot.id);
        return;
      }
  ...
  getMatchUpgradeOffer(robotId) {
    const ids = this.matchUpgradeOffers?.[robotId] || [];
    return ids
      .map((id) => CONFIG.UPGRADES.find((upgrade) => upgrade.id === id))
      .filter(Boolean);
  }
  ...
  setMatchReady(robotId) {
    ...
    if (!this.matchReadyRobotIds.includes(robot.id)) {
      ...
      this.pushLog(`${robot.name} heeft een start-upgrade gekozen.`);
    }
    delete this.matchUpgradeOffers?.[robot.id];
  ...
  getCardPoolForRobot(robot = null) {
    // Handkaarten worden enkel getrokken uit de basiskaarten (MOVE 1/2/3, Back up, Turn L/R, U-Turn).
    // Speciale kaarten (MOVE 4, Krab Move, Jump) kunnen uitsluitend via Merge worden gemaakt als je de bijbehorende upgrade bezit.
    return (CONFIG.CARD_TYPES || []).filter((c) => c.type !== 'wait' && (c.weight == null || c.weight > 0));
  }

  generateProgramCard(robot = null) {
    const pool = robot
      ? this.getCardPoolForRobot(robot)
      : (CONFIG.CARD_TYPES || []).filter((c) => c.type !== 'wait' && (c.weight == null || c.weight > 0));
    
    // Weighted random selection: most common = move1, turnL, turnR; mid = move2, backup; least = move3, uturn
    ...
    const prioRoll = this.isP2pMode() ? this.rng() : Math.random();
    return {
