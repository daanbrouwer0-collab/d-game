class RobotRallyUI {
  constructor(canvas, engine) {
    ...
    this.localMatrixRobotId = null;
    this.matrixHostMode = false;
  ...
  isMatrixMode() {
    return !!(this.engine?.isMatrixMode?.() || this.engine?.gameMode === CONFIG.GAME_MODES.MATRIX);
  }

  isMatrixHost() {
    return this.isMatrixMode() && !!(this.matrixHostMode || MatrixSessionController?.isHost?.());
  }
  ...
  updatePanelTitle(currentRobot) {
    ...
    if (this.engine.phase === 'programming') {
      ...
      if (this.selectionHint) {
        ...
        if (this.isMatrixMode()) {
          ...
          this.selectionHint.textContent = `Matrix: programmeer tegelijk. Ready ${readyCount}/${total}.${powerDownText}`;
        } else {
    ...
    if (this.engine.phase === 'ready') {
      ...
      if (this.selectionHint) {
        this.selectionHint.textContent = this.isMatrixMode()
          ? (this.isMatrixHost()
  ...
  renderPlayerStatusBar() {
    ...
    this.playerStatusBar.innerHTML = robots.map(robot => {
      ...
      const isTurn = this.isMatrixMode()
      ...
      const isReady = this.isMatrixMode() && this.engine.isRobotCommitted?.(robot.id);
  ...
  updateButtons() {
    ...
    if (this.btnConfirmProgram) {
      const alreadyCommitted = this.isMatrixMode() && currentRobot && this.engine.isRobotCommitted?.(currentRobot.id);
  ...
  runProgram() {
    if (this.engine.phase !== 'programming' && !(this.isMatrixMode() && this.engine.phase === 'ready')) return;
    const robot = this.getProgrammingRobot();
    if (!robot) return;
    if (this.isMatrixMode() && this.engine.isRobotCommitted?.(robot.id)) {
    ...
    if (this.isMatrixMode() && MatrixSessionController?.isActive?.()) {
      MatrixSessionController.sendCommit(this.selectedRegisters)
        .then(() => {
          this.selectedRegisters = [null, null, null, null, null];
          this.programmingRegistersRobotId = null;
          this.updateCardsUI();
  ...
  getProgrammingRobot() {
    if (this.isMatrixMode() && this.localMatrixRobotId) {
      return this.engine.robots.find(robot => robot.id === this.localMatrixRobotId)
        || this.engine.robots.find(robot => robot.matrixUserId === MatrixSessionController?.auth?.userId)
        || null;
    }
  ...
  syncExecutionTimer() {
    ...
    if (this.isMatrixMode() && !this.isMatrixHost()) {
      if (this.autoStepTimer) {
        clearInterval(this.autoStepTimer);
        this.autoStepTimer = null;
      }
  ...
  renderPlaybackOverlay() {
    ...
    const matrixGuestReady = this.isMatrixMode() && readyMode && !this.isMatrixHost();
    ...
    if (this.btnStartExecution) {
      this.btnStartExecution.disabled = matrixGuestReady;
      ...
      this.btnStartExecution.classList.toggle('rainbow-play-btn', readyMode && !matrixGuestReady);
      ...
      if (privacyLocked && currentRobot) {
        ...
        this.btnStartExecution.textContent = matrixGuestReady ? 'Wachten op host…' : 'Play ▶';
      } else {
        this.btnStartExecution.style.removeProperty('--player-btn-color');
        this.btnStartExecution.style.removeProperty('--player-btn-color-dark');
      }
  ...
  renderUpgradeChoiceOverlay() {
    ...
    this.upgradeChoiceList.querySelectorAll('[data-upgrade-choice]').forEach(button => {
      button.addEventListener('click', () => {
        ...
        if (!upgradeId) return;
        if (this.isMatrixMode() && MatrixSessionController?.isActive?.()) {
          MatrixSessionController.sendUpgrade(upgradeId)
  ...
  startExecution() {
    ...
    if (this.isMatrixMode()) {
      if (!this.isMatrixHost()) {
        Toast.show('Alleen de host kan Play drukken.');
        return;
      }
      MatrixSessionController.sendPlay()
  ...
  isProgrammingUnlocked(robot = this.getProgrammingRobot()) {
    if (this.isMatrixMode()) return this.engine.phase === 'programming' || this.engine.phase === 'ready';
  ...
  isPrivacyGateVisible(robot = this.getFocusedRobot()) {
    if (this.isMatrixMode()) return false;
