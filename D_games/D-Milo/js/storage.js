...
const Storage = (() => {
  ...
  function forgetAll() {
    try {
      const toRemove = [];
      const exactKeys = new Set(Object.values(keys));
      const slugPrefix = `game:${GameConfig.slug}:`;

      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (!k) continue;
        if (exactKeys.has(k) || k.startsWith(slugPrefix)) {
          toRemove.push(k);
        }
      }
  ...
  function readUsername() {
    return String(readJson(keys.username, '') || '').trim();
  }

  function writeUsername(name) {
    return writeJson(keys.username, String(name || '').trim());
  }

  function readSettings() {
    return readJson(keys.settings, { sound: false, vibration: true, characterColors: null, lastSessionId: null });
  }
