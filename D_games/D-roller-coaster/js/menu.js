...
const Menu = (() => {
  ...
  function applySessionToGame(session, fromSave = false) {
    ...
    RollerCoasterGame.setSessionMeta({
    ...
    if (fromSave && session.gameState) {
      RollerCoasterGame.importState(session.gameState);
      hasStarted = true;
      hide();
      RollerCoasterGame.start(true);
